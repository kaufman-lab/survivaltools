...
## Basic data functions: enrollment, age,

Create a function to generate basic data (enrollment entry)
```{r}


## a function which generates ppt table containing
 #t_tstart,t_exit (ie, end of follow-up date), and baseline_age according
 #to specified distributions
##createa  table with 3 columns: ppt_id,
 #randomly generated enrollement date ("t_start"),
 #a randomly generated baseline age ("baseline_age")
 #a fixed study end date (last possible observable day for all ppts)

create_ppt_table <- function(n){
  out <-  data.table(
    ppt_id=1:n,
    baseline_age=as.integer(floor(
...
###h0(x) is the baseline hazard function and h(x|X)=h0(x)*exp(XB)
#eg:
baseline_hazard=function(x,ppt){0.000015}
total_hazard2 <- function(x){baseline_hazard(x)*exp((1/20000)*x)}
total_hazard2(365*70)/total_hazard2(365*50)
 #20 years of aging is a HR of ~1.44=exp(20*365*1/20000)
total_hazard3 <- function(x,s){baseline_hazard(x)*exp((1/20000)*x+2*s)}
total_hazard3(365*70,1)/total_hazard3(365*70,0) 
total_hazard3(365*70,1)/total_hazard3(0,1) 
total_hazard3(365*70,1)/total_hazard3(0,0) 

#smoking RR~7.4=exp(2)
```

to do better simulations we'd like a baseline hazard which isn't constant over time and is different
for each simulation run.

Here's a function which generates random baseline hazards for every day for every participant.
Note that the baseline hazard is the same for everyone on a given day, but this function 
could easily be adapted to have different baseline hazards for different strata.

These hazard functions are generated via random points+spline fits, specifically chosen 
to generate a reasonable baseline hazard (on average about 0.000015 probability of an event a day
with temporally correlated variations above and below that over time)
...
note that the resulting hazard function is a function of both ppt_id and time, but the hazard is the same
for all ppts. 

since the hazard function is the same for everyone, I'm just picking an arbitrary ppt (ppt_id=1) to generate
some example hazard functions:


plot some baseline hazards:
...
par(mfrow=c(1,1))
  ...
  for(i in 1:6){
    ...
    if(i==1){
      hazard_table1[J(i), plot(time,baseline_hazard,type="l",col=i,ylim=c(1e-5,2e-5)),on="ppt_id"]
    } else{
      hazard_table1[J(i), lines(time,baseline_hazard,type="l",col=i,ylim=c(1e-5,2e-5)),on="ppt_id"]
    }
  }
```

Note that these hazard occur over a very tight range around ~0.000015
This is the baseline probabiltiy of an event occuring on a single day.

### time-varying variables:  exposure

Next let's generate some time-varying exposures. e.g. something like PM2.5.

We'll use the conceptual framework of the spatiotemporal model to generate exposures with 
some set of temporal trends common to everyone and each individual has a randomly generated coefficient
to that temporal trend. Note that I'm not explicitly modeling space here 
(just picking beta-field values randomly) and as such it's not possible to induce spatial confounding
in the simulation (just temporal confounding).

The first temporal trend is constant, the second and third temporal trends are random splines, and the 
third temporal trend is a linear function of time.

The coefficients of the first temporal trend have mean 18. the spline coefficeints have mean zero.
The coefficients of the linear term are mean -0.0009 (creating a linear secular trend on top of shorter 
term variations.)
...
create_exposure_table <- function(ppt_tbl,n_years=2L){
  ...
  b <- cbind(rnorm(nrow(ppt_tbl),mean=18,sd=1), 
             ...
             rnorm(nrow(ppt_tbl),sd=.4),
             rnorm(nrow(ppt_tbl),-0.0009,sd=.0001)
  )
...
Visualize some explosure plots. each color is a ppt. each pane is a different set of exposure functions in a different set of ppts
...
```
I probably could have started out with a smoother trend and called that a two-year average! The daily
predictions really serve no purpose in this simulation. But they look nice!
...
## Survival simulation: time-varying exposure
