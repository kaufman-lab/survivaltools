...
hazard_table1 <- generate_hazard_table(ppt_tbl)
plot(hazard_table1[ppt_id==1,]$time,hazard_table1[ppt_id==1,]$baseline_hazard,type="l",ylim=c(2e-6,3.5e-6))
```

Some details about the specifics of the above code: 
Note that `generate_hazard_table` function generates the same baseline hazard for function for everyone, but is stored such that every participant has their own set of hazard values for every day. (This formulation of the hazard function/dataset is a good format to input into simulation code because this format would generalize to a situation where different subjects have different baseline hazards). 



`generate_hazard_table` will generate a new set of baseline hazards every time it's called. Here we plot several different baseline hazards:
...
Visualize some explosure plots. each color is a ppt. each panel is a different set of exposure functions in a different set of ppts. Note that these are supposed to symbolize 2-year averages so there's not that much variabililty.
...
for(j in 1:4){
  ppt_tbl <- generate_ppt_table(1000)
  exposure_tbl <- generate_exposure_table(ppt_tbl,dofoverlaps=FALSE)
  exposure_tbl <- exposure_tbl[between(time,min(ppt_tbl$t_start),max(ppt_tbl$t_exit))]
  exposure_tbl[, plot(1,1,type="n",xlim=range(time)+c(-1,1)*diff(range(time))*.1,ylim=c(8,22))]
  for(i in 1:10){
    exposure_tbl[J(i), lines(x=time,y=exposure_2yravg,col=i),on="ppt_id"]
  }
}
  par(mfrow=c(1,1))
...
No pm2.5 exposure, but include an effect of aging.
...
results <- parLapply(cl, 1:200,function(i){
  ...
  ppt_tbl1 <- generate_ppt_table(2000)
  ppt_tbl1[, smoking:=rbinom(.N,size=1,prob=.5)]
  hazard_table1 <- generate_hazard_table(ppt_tbl1)

  q <- simulate_survival(x=ppt_tbl1,
                         z=hazard_table1,
                         x_covar_names="smoking",
                         z_tv_covar_names = c("age_since65_tv"),
                         covar_coefs=1.2,
                         tv_covar_coefs=c(1/10000)
 )
  ...
  out <- list(
    extract_coef_table(coxph(Surv(start,end,event)~strata(baseline_age_c)+smoking,data=q)),
    extract_coef_table(coxph(Surv(start,end,event)~baseline_age_c+smoking,data=q)),
    extract_coef_table(coxph(Surv(start,end,event)~smoking,data=q))
  )
...
As expected, results are uniased regardless of whether we stratify, adjust, or ignore age.


## Survival simulation with a time-varying variable
...
cl <- makeCluster(getOption("cl.cores",n_cores ))
clusterExport(cl, c("generate_ppt_table","generate_hazard_table","generate_exposure_table"))
results1 <- parLapply(cl, 1:39,function(i){
  ...
  ppt_tbl1[, smoking:=rbinom(.N,size=1,prob=.5)]
  tv_table <- generate_hazard_table(ppt_tbl1)
  exposure_table1 <- generate_exposure_table(ppt_tbl1)
...
results3 <- parLapply(cl, 1:39,function(i){
  ...
  ppt_tbl1 <- generate_ppt_table(1000) #increase sample size from previous examples
...
## ref
understanding non-proportional hazards:
