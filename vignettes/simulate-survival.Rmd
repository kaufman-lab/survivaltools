...
Add a time-varying covariate as
...
cl <- makeCluster(getOption("cl.cores",n_cores ))
results <- parLapply(cl, 1:2000,function(i){
  library(data.table)
  setDTthreads(1)
  library(survivaltools)
  library(survival)
  ...
  formulas <- list(Surv(t_start,t_end,event)~strata(baseline_age_c)+exposure,
       Surv(t_start,t_end,event)~baseline_age_c+exposure,
       Surv(t_start,t_end,event)~baseline_age_c*baseline_year+exposure,
       Surv(t_start,t_end,event)~baseline_age+exposure,
       Surv(t_start,t_end,event)~baseline_age*baseline_year+exposure,
       Surv(t_start,t_end,event)~exposure
       )
